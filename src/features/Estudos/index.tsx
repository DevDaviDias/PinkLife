...
import StatusCard2 from "@/src/componentes/ui/StatusCard2";
...
export default function Estudos() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const [resMaterias, resHistorico] = await Promise.all([
          axios.get(`${API_URL}/estudos/materias`, { 
            headers: { Authorization: `Bearer ${token}` } 
          }),
          axios.get(`${API_URL}/estudos/historico`, { 
            headers: { Authorization: `Bearer ${token}` } 
          })
        ]);
  ...
  const handleTabChange = (tab: "Hoje" | "Semana" | "Historico") => {
    setActive(tab);
    // Scroll suave para o conteúdo ao trocar de aba
    setTimeout(() => {
  ...
  async function excluirMateria(id: string) {
    ...
    try {
      // Atualiza a lista completa no backend (padrão que definimos no index.js)
      await axios.post(
  ...
  // Esta é a função que o CronometroEstudos chama via 'onFinalizar'
  async function finalizarSessao(sessao: StudySession) {
    ...
    try {
      ...
      // Atualiza as horas estudadas da matéria localmente para feedback imediato
      setMaterias((prev) =>
        prev.map((m) =>
  ...
  return (
    <ContainerPages>
      ...
      <GrayMenu
        items={[
          { title: "Minhas Matérias", onClick: () => handleTabChange("Hoje"), active: active === "Hoje" },
      ...
      <div className="mt-4" ref={conteudoRef}>
        {loading ? (
           <div className="flex flex-col items-center justify-center py-20">
             ...
             <p className="mt-4 text-pink-500 font-medium">Sincronizando estudos...</p>
           </div>
        ) : (
          <div className="animate-in fade-in duration-500">
            ...
            {/* ABA: MINHAS MATÉRIAS */}
            {active === "Hoje" && (
              <StatusCard width="" title="Gerenciar Matérias">
                  ...
                  {materias.map((m) => (
                    <div key={m.id} className="relative group">
                      <Cardprogresso
                        ...
                        progresso={m.metaHoras > 0 ? Math.min(((m.horasEstudadas || 0) / m.metaHoras) * 100, 100) : 0}
                        barraDeProgresso
            ...
            {/* ABA: HISTÓRICO */}
            {active === "Historico" && (
              <StatusCard width="h-full" title="Sessões Recentes">
                <div className="space-y-3">
                  {historico.length === 0 && (
                    <p className="text-center text-gray-400 py-10">Você ainda não realizou nenhuma sessão.</p>
                  )}
                  {historico.map((h) => (
                    <div key={h.id || Math.random().toString()} className="border-l-4 border-pink-400 p-4 rounded-r-lg bg-white shadow-sm flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-sm font-bold text-pink-500 mt-1">
                          ⏱️ {(h.duracaoSegundos / 60).toFixed(0)} minutos focados
                        </p>
                      </div>
                    </div>
                  ))}
